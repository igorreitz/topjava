...
//        .toLocalTime();
    ...
    public static List<UserMealWithExceed> getFilteredWithExceededByStream(List<UserMeal> mealList, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        Map<LocalDate,Integer> caloriesPerDate= mealList.stream().collect(Collectors.groupingBy(s -> s.getDateTime().toLocalDate(), Collectors.summingInt(UserMeal::getCalories)));
    ...
    public static List<UserMealWithExceed> getFilteredWithExceededByStreamOptional2(List<UserMeal> mealList, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        return mealList
                ...
                .flatMap(userMealListPerDate -> {
                    boolean exceed = userMealListPerDate.stream().mapToInt(UserMeal::getCalories).sum() > caloriesPerDay;
                    return userMealListPerDate.stream().filter(meal -> TimeUtil.isBetween(meal.getTime(), startTime, endTime)).map(meal -> getNewUserMealWithExceed(meal,exceed));
                }).collect(Collectors.toList());
    }
